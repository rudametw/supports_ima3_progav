...
%		\begin{block}{Tableaux}
		...
		\node [align=center, anchor=south] at (array-2-9.north west|-first.south) (8) {Élément\\ (à l'indice 8)};
		\draw (8)--(box);
		%
		\end{tikzpicture}
...
typedef <un_type> <synonyme_du_type>
		...
		\vspace{-0.3cm}
		\begin{block}{}
			\item Donnez des noms plus simples pour faciliter l'écriture et augmenter la lisibilité 
		\end{block}
		\begin{block}{}
			\item Examples :
			\begin{minted}[
...
typedef struct ma_structure * ptr_ma_struct;
typedef struct S S;
			\end{minted}
		\end{block}
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{block}
		\vspace{1cm}
		\begin{block}{\Large Beaucoup de choses à programmer\\ \textit{à la main} !!!}
		\end{block}		
	\end{itemize}
\end{frame}
	...
	\begin{itemize}
		\begin{block}{}
			...
			\begin{itemize}
				...
				\item Ou utilisez les fonctions de C dédiées : \texttt{strcpy, strncpy, strncat, \ldots}
			\end{itemize}
			\vspace{0.5cm}
			\begin{adjustwidth}{-1.6em}{-1.5em}
				\begin{minted}[
...
/* Ou concaténer avec une chaine vide: */
...
\begin{frame}[fragile=singleslide]
	\frametitle{Tableaux de structures: initialisation avancée }
	\vspace{-0.7cm}
	\begin{itemize}
		\begin{block}{}
